...
export default function EmployeePortal() {
    ...
    const initiateAttendance = (empId, empName) => {
        // Geofencing Check (Skip if key override is active for testing or disabled in config)
        if (!isAdminOverride && !OFFICE_LOCATION.disableLocationCheck) {
            ...
            // Strict check only for Clock In? 
            // Usually Clock Out is allowed from anywhere, but let's keep it strict or consistent as per user request context.
            // Original code enforced it for the whole toggle. 
            // If we want to allow remote clock-out, we'd check if (isClockedIn) skip check.
            // For now, preserving original restrictive behavior.
    ...
    const handlePinSubmit = (e) => {
        ...
        // Check PIN (If old employee has no pin, maybe allow default or require update? 
        // For security, if no PIN is set, we might fallback or block. 
        // Let's assume strict: Must have PIN. If undefined, maybe allow '0000' or block.
        // Blocking is safer. User can update employee in Admin.)
    ...
    const executeAttendance = (empId, empName) => {
        ...
        history.push({
            ...
            location: currentLocation, // Save verified location
            status: isCurrentlyClockedIn ? '-' : attendanceStatus // Save Late/OnTime status
        });
        ...
        activity.unshift({
            id: Date.now(),
            text: `${empName} ${isCurrentlyClockedIn ? 'Clocked Out' : 'Clocked In'} ${!isCurrentlyClockedIn && attendanceStatus === 'Late' ? '(Late ⚠️)' : ''}`,
    ...
    const handleLeaveSubmit = (e) => {
        ...
        activity.unshift({
            id: Date.now(),
            text: `${selectedLeaveEmp.name} requested leave`,
            time: new Date().toLocaleTimeString(),
            type: 'leave'
        });
        localStorage.setItem('activity_log', JSON.stringify(activity));
                    ...
                    <span>
                        {locationError ? locationError :
                            distanceToOffice === null ? 'Locating...' :
                                OFFICE_LOCATION.disableLocationCheck ? 'Location Check Disabled (Demo Mode)' :
                                    isWithinRange ? `You are at the office (${distanceToOffice}m)` :
                                        `You are too far (${distanceToOffice}m). Office is ${OFFICE_LOCATION.allowedRadiusMeters}m range. (Detected: ${currentLocation?.latitude?.toFixed(4)}, ${currentLocation?.longitude?.toFixed(4)})`}
                    </span>
                </div>
            </div>
