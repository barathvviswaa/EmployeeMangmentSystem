...
export default function Admin() {
    ...
    useEffect(() => {
        const loadData = () => {
            ...
            if (savedEmps) {
                // Only update if different to avoid rerenders loop (basic check)
                const parsed = JSON.parse(savedEmps);
                if (JSON.stringify(parsed) !== JSON.stringify(employees)) {
                    setEmployees(parsed);
                }
            }

            const active = JSON.parse(localStorage.getItem("attendance_record")) || [];
            if (JSON.stringify(active) !== JSON.stringify(activeIds)) setActiveIds(active);
            ...
            if (JSON.stringify(history) !== JSON.stringify(attendanceHistory)) setAttendanceHistory(history);
        };

        // Initial load only relies on state initializer usually, but this ensures other data syncs
        loadData();
    ...
    }, []); // Empty dependency mainly, but we need to be careful with closure if we used state inside loadData. Here we read from localStorage.
    ...
    const exportToCSV = () => {
        ...
        const rows = employees.map(e => `${e.id},${e.name},${e.role},${e.department || 'N/A'},${e.salary},${new Date(e.id).toLocaleDateString()}`);
